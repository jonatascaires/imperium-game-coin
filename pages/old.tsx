...
export default function Index() {
  ...
  const renderContent = useCallback(() => {
    switch (activePage) {
      ...
      case 12:
        return <div className="overflow-y-auto h-[675px] my-3 animate__animated animate__fadeIn"><Presale setActivePage={setActivePage} contract={contract} address={address} checkBalance={checkBalance} language={language} /></div>
  ...
  return (
    <>
      ...
      <div className={`flex absolute top-2 h-8 justify-center items-center gap-6 font-OdibeeSans w-screen text-xl text-white ${activePage > 0 && 'hidden'}`}>
        ...
        <a href="https://imperiumtruck.com"><div className="hidden sm:block cursor-pointer animate-pulse opacity-80 hover:opacity-100">{language === 'en' ? 'Beta version' : 'Versão beta'}</div></a>
      </div>
